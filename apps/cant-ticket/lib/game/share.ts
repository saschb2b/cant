...
export function getRank(percentage: number): string {
  if (percentage >= 90) return "Design Lead";
  if (percentage >= 70) return "UX Craftsperson";
...
export function getShareUrl(state: GameState): string {
  return `https://cant-ticket.saschb2b.com/play/results?r=${encodeResults(state)}&seed=${state.seed}`;
}

...
export function getRank(percentage: number): string {
  if (percentage >= 90) return "Game Master";
  if (percentage >= 70) return "Senior Dev";
...
export function getShareUrl(state: GameState): string {
  return `https://cant-trust.saschb2b.com/play/results?r=${encodeResults(state)}&seed=${state.seed}`;
}

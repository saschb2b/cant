...
export function getRank(percentage: number): string {
  if (percentage >= 90) return "Query Master";
  if (percentage >= 70) return "Endpoint Architect";
...
export function getShareUrl(state: GameState): string {
  return `https://cant-query.saschb2b.com/play/results?r=${encodeResults(state)}&seed=${state.seed}`;
}

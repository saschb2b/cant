...
export function getRank(percentage: number): string {
  if (percentage >= 90) return "Test Master";
  if (percentage >= 70) return "Quality Architect";
...
export function getShareUrl(state: GameState): string {
  return `https://cant-test.saschb2b.com/play/results?r=${encodeResults(state)}&seed=${state.seed}`;
}

...
export function getRank(percentage: number): string {
  if (percentage >= 90) return "Lab Director";
  if (percentage >= 70) return "Senior Chemist";
...
export function getShareUrl(state: GameState): string {
  return `https://cant-explode.saschb2b.com/play/results?r=${encodeResults(state)}&seed=${state.seed}`;
}

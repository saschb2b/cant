...
interface SearchPaletteConfig {
  /** Placeholder text for the search input. */
  placeholder: string;
  /** Hint shown when no results are found. */
  noResultsHint: string;
  /** Group header for challenge/pattern results. */
  challengeGroupLabel: string;
  /** Extra page icons beyond the defaults (FileText, BookOpen, Code, GraduationCap). */
  pageIcons?: Record<string, LucideIcon>;
}
...
const typeIconMap: Record<SearchItem["type"], LucideIcon> = {
  page: FileText,
  category: BookOpen,
  challenge: Code,
};

const defaultPageIconMap: Record<string, LucideIcon> = {
  learn: GraduationCap,
};
...
export function SearchPalette({
  ...
  const pageIconMap = useMemo(
    () => ({ ...defaultPageIconMap, ...config.pageIcons }),
    [config.pageIcons],
  );

  function getIcon(item: SearchItem): LucideIcon {
  ...
  const browseItems = useMemo(() => {
    ...
    const flat = [...pages, ...sectionList.flatMap((s) => s.items)];
    return { pages, sections: sectionList, flat };
  }, [items, sections]);
  ...
  const handleKeyDown = useCallback(
    ...
    [activeList, highlightedIndex, navigate, query, trackEvent],
  );
            ...
            {visibleChallenges.length > 0 && (
              <>
                ...
                {hiddenChallengeCount > 0 && (
                  <Chip
                    label={`Show ${String(hiddenChallengeCount)} more ${config.challengeGroupLabel.toLowerCase()}`}
                    size="small"
                    variant="outlined"
